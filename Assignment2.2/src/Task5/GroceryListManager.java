...
public class GroceryListManager {
    ...
    private HashMap<String, Task5.GroceryListManager.Item> groceryList = new HashMap<>();
    ...
    public void displayByCategory(String category) {
        ...
        for (Map.Entry<String, Item> entry : groceryList.entrySet()) {
            if (entry.getValue().category.equalsIgnoreCase(category)) {
                System.out.printf("%d. %s - $%.2f%n", i++, entry.getKey(), entry.getValue().cost, entry.getValue().quantity);
                found = true;
            }
        }
        if (!found) {
    ...
    public double calculateTotalCost() {
        double total = 0.0;
        for (Task5.GroceryListManager.Item item : groceryList.values()) {
    ...
    public static void main(String[] args) {
        Task5.GroceryListManager manager = new GroceryListManager();

        // Test 1: Add items with different costs and quantities
        manager.addItem("Apples", 2.50, "Fruits", 5);
        manager.addItem("Bananas", 1.20, "Fruits", 3);
        manager.addItem("Milk", 1.75, "Dairy", 2);
        manager.addItem("Bread", 2.00, "Bakery", 1);
        manager.addItem("Cheese", 3.50, "Dairy", 4);
        manager.displayList();

        // Test 2: Total cost calculation (considering quantity)
        double total = manager.calculateTotalCost();
        System.out.printf("\n Total cost of all items: $%.2f%n", total);

        // Test 3: Display items by category
        ...
        manager.updateQuantity("Bread", 0);

        // Show available items
        manager.displayAvailableItems();

        // Test 5: Recalculate total cost after quantity updates
        total = manager.calculateTotalCost();
        System.out.printf("\n Updated total cost: $%.2f%n", total);
    }
}
